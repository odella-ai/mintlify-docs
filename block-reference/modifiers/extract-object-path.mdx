...
<ParamField path="Object" type="object | any[] | string">
  The object, array, or string to query using the JSONPath. Required.
</ParamField>

## Outputs

<ResponseField name="Match" type="any">
  The first value that matches the specified path. If no match is found, this output will not run.
</ResponseField>

<ResponseField name="All Matches" type="any[]">
...
<ParamField path="Path" type="string" default="$">
  The JSONPath query to use for extracting properties from the input object. This setting can be toggled to accept input from another block.
</ParamField>
...
<Frame>
  <img src="./assets/extract-object-path-block-example-01.png" alt="Extract Object Path Block Example" />
</Frame>

## Error Handling

- If the path is invalid, the block will throw an error.
- If the path is valid but no value is found, the `Match` output will not run, and the `All Matches` output will run with an empty array.
