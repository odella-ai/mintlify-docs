...
<ParamField path="Array" type="any[]">
  The array to pop from. Required. Non-array inputs will be coerced into an array.
</ParamField>

## Outputs

<ResponseField name="Last" type="any">
  The popped element (last or first, depending on settings).
</ResponseField>
...
<ParamField path="Pop from front" type="boolean" default={false}>
...
<Frame>
  <img src="./assets/pop-block-example-01.png" alt="Pop Block Example 1" />
</Frame>
...
3. Run the flow. The `Last` output should be `"John"` and the `Rest` output should be `["Doe", "30", "Engineer"]`.

<Frame>
  <img src="./assets/pop-block-example-02.png" alt="Pop Block Example 2" />
</Frame>
...
- The input is not an array (and cannot be coerced into one)
