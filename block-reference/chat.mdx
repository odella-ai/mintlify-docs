...
The Chat Block send one or more messages to an LLM - OpenAI's GPT, or any API compatible with the OpenAI API. It then returns the response from the LLM.

You can use the Chat Block for local LLMs, as long as their API is compatible with the OpenAI API. For example, you can use the Chat Block with [LM Studio](https://lmstudio.ai/).

If you are looking for other language models that do not support the OpenAI API format, see the Plugins page for a list of available plugins that implement other language model blocks.

![Chat Block Screenshot](./assets/chat-block.png)
...
| -------- | --------- | ---------------------- | ----------------------------------------------------------------------------------------------------------- |
| Response | string    | The response from GPT. | The response will be streamed in, but subsequent blocks will not be executed until the response is finished. |
...
| Number of Choices               | Allows the Chat Block to output a list of responses simultaneously if the value is greater than 1. Each response will have a unique set of text (assuming the temperature is more than 0)                                                                                                                | 1                                                                                                                           | Yes              | `number`        |
| Enable Function Use             | Allows the use of functions in the prompt. Enables the `Functions` input.                                                                                                                                                                                                                               | false                                                                                                                       | No               | N/A             |
| Cache                           | Caches the response locally in Lawme. If the chat block gets the exact same prompt again, it will return the cached response instead of making a new request to OpenAI.                                                                                                                                  | false                                                                                                                       | No               | N/A             |
| Use for subflow partial output | If enabled, the Chat Block will be used to generate partial output for subflows. This is only visual - the chat block's partial output will appear as the subflow's partial output. Only enable if the chat block will be running exclusively from other chat blocks at the same time, to avoid problems. | true                                                                                                                        | No               | N/A             |
| Endpoint                        | The endpoint to use for the request. For example, you can set the endpoint to `http://localhost:1234/v1/chat/completions` for LM Studio. You may also set a global Chat Block endpoint int the Lawme Settings page.                                                                                      | (empty string, uses the default endpoint configured in Settings, or OpenAI's chat completions endpoint if that is not set.) | No               | `string`        |
...
1. Add a Chat block to your flow.
2. Add a text block and place your message to GPT inside the text block by opening its editor and replacing `{{input}}` with your message.
3. Connect the output of the text block to the `Prompt` input of the Chat block.
4. Run your flow. You will see the output of the Chat block at the bottom of the block.

![Simple Response Example](./assets/chat-block-example-01.png)
...
7. Run your flow. You will see the output of the Chat block at the bottom of the block.

## Error Handling

If nothing is connected to the `Prompt` input, the Chat block will error.

If the request to OpenAI fails due to rate-limiting, the Chat block will retry the request using a jittered exponential backoff algorithm. This retry will
happen for up to 5 minutes. If the request still fails after 5 minutes, the Chat block will error.

:::caution

Be careful [batching](../user-guide/workflow/batching.md) a Chat block too much that you run into rate limiting issues.

:::

If OpenAI returns a 500-level error (due to being overloaded or downtime, etc), the Chat block will retry in a similar manner.
...
A: The Chat Block will error. The prompt input is required. To send only a system prompt, you can use a [Prompt](./prompt.mdx) block to create a system-type prompt, and connect it to the `Prompt` input.
...
- [Assemble Prompt Block](./assemble-prompt.mdx)
- [Prompt Block](./prompt.mdx)
- [Text Block](./text.mdx)
