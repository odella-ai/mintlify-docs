...
<Frame>
  <img src="/images/audio-block.png" alt="Audio Block Screenshot" />
</Frame>

## Inputs

<ParamField path="Data" type="string">
  The string representing the audio data. This input is only available if the "Use Data Input" setting is enabled. The input will be coerced into a string.
</ParamField>

## Outputs

<ResponseField name="Audio Data" type="audio">
  The audio data converted into a format that can be used by other blocks that accept audio data as input.
</ResponseField>

## Editor Settings

<ParamField path="Audio File" type="file">
  The audio file to be used as the audio data. This file should be in a format that can be converted to a Uint8Array.
</ParamField>
