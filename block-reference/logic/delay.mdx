...
<ResponseField name="Output[i]" type="any">
  The ith output from the execution of the delay block. Each output corresponds to its respective input and is returned after the specified delay.
</ResponseField>
...
<ParamField path="Use Delay Input" type="boolean" default={false}>
  If enabled, the block will use the "Delay" input as the delay time. If disabled, it will use the delay time specified in the "Delay" setting.
</ParamField>
