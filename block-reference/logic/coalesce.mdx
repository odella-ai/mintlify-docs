...
2. For each case, use an [If Block](/block-reference/logic/if-else) to transform each case into a unique value, say `A`, `B`, and `C`. Use a [Text Block](/block-reference/data/text) connected to the `value` port of the If Blocks to set the value passed through for each case.
