...
This is where you connect the applications your team already uses so Odella can work with them directly.
...
A **Search plugins...** bar sits at the top of the page, above a searchable list of individual plugins. Each row has a toggle that turns the plugin on or off for your workspace.

{/* TODO: what scopes does each connection grant? Are credentials per-workspace or per-user? What happens when a token expires mid-workflow? */}
