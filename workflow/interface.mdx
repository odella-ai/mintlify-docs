...
description: "Navigate the workflow canvas, panels, controls, settings, and execution tools."
---
...
The workflow interface brings the canvas, block library, run controls, and configuration panels into one workspace.
...
<CardGroup cols={2}>
  ...
  >
    The main workspace where you design and visualize a workflow.
  </Card>
  <Card
    title="Block Editor"
    icon="pen-to-square"
  ...
  >
    Customise individual blocks to fine-tune your workflow's behavior.
  </Card>
  <Card
    title="Action Bar"
    icon="play"
...
Save draft changes before leaving the workflow or sharing it with another editor.
...
<Tip>
  Connected applications add provider-specific operations to the workflow builder.
</Tip>
...
Use the Flow Canvas to arrange, connect, and inspect the steps in a workflow:
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="sliders"
  >
    Access specialized editors tailored to each block type.
  </Card>
</CardGroup>
