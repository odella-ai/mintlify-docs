...
description: 'Master the art of creating, managing, and executing AI-powered workflows in Odella'
---

 

<Callout type="info">
  Flows are the cornerstone of Odella, representing sophisticated business processes and AI-powered workflows that revolutionise legal task automation.
</Callout>

A Odella project is a collection of multiple flows. Each flow serves as a visual representation of a legal task or process, constructed using interconnected blocks that collaborate to achieve specific outcomes.

## Understanding Flows

Flows in Odella are designed to streamline and accelerate your legal work. They function as a chain of intelligent building blocks, with each block performing a specialised task. Information cascades through these blocks from left to right, enabling you to craft smart, AI-powered workflows without any coding expertise.
...
<Frame>
  <img src="../assets/workflows_new_light.gif" alt="Workflow Creation Process (Light Mode)" className="block dark:hidden" />
  <img src="../assets/workflows_new_dark.gif" alt="Workflow Creation Process (Dark Mode)" className="hidden dark:block" />
</Frame>
...
<Frame>
  <img src="../assets/workflows_delete_light.gif" alt="Workflow Deletion Process (Light Mode)" className="block dark:hidden" />
  <img src="../assets/workflows_delete_dark.gif" alt="Workflow Deletion Process (Dark Mode)" className="hidden dark:block" />
</Frame>
...
<Frame>
  <img src="../assets/workflows_run_light.gif" alt="Workflow Running Process (Light Mode)" className="block dark:hidden" />
  <img src="../assets/workflows_run_dark.gif" alt="Workflow Running Process (Dark Mode)" className="hidden dark:block" />
</Frame>
...
Flows in Odella are designed as a graph-like structure, where inputs and outputs are determined by specific blocks within the workflow. This approach enhances flexibility and reusability across various scenarios.
...
<CardGroup cols={2}>
  <Card title="Input Blocks" icon="arrow-right-to-bracket">
    [Input Blocks](/block-reference/io/graph-input) serve as entry points for data in your flow. They define the parameters your workflow requires to run.
  </Card>
...
<CardGroup cols={2}>
  <Card title="Output Blocks" icon="arrow-right-from-bracket">
    [Output Blocks](/block-reference/io/graph-output) define the results your flow returns upon completion. They act as exit points for processed data.
  </Card>
...
By mastering flows in Odella, you'll be equipped to create powerful, AI-driven workflows that automate complex legal tasks and significantly enhance your firm's efficiency.
