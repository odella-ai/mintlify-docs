---
title: "Blocks"
description: "Add, connect, configure, group, and remove blocks on the Odella workflow canvas."
---

 

Blocks are the individual steps in a workflow. Each block accepts defined inputs, performs one operation, and returns outputs for downstream steps.

## Adding a Block

To add a block, right-click in any empty space on the flow canvas (or press **Space**) to open the **Add Menu**.

<Frame>
  <img src="/assets/block_add_light.gif" alt="Adding a block (Light Mode)" className="dark:hidden" />
  <img src="/assets/block_add_dark.gif" alt="Adding a block (Dark Mode)" className="hidden dark:block" />
</Frame>

<CardGroup cols={2}>
  <Card title="Search & Select">
    Start typing to quickly find the block you need in the menu.
  </Card>
  <Card title="Organised by Category">
    Blocks are grouped by category for easy browsing. Select a block to place it at the location you right-clicked.
  </Card>
</CardGroup>

For details on all available blocks, check out the [Block Reference](/block-reference/all-blocks).

## Moving Blocks

To reposition blocks:

- **Drag & Drop**: Click and hold the title bar of a block, then drag it to your desired position.
- **Multi-Select**: Hold **Shift** and click on multiple blocks to move them as a group.

<Frame>
  <img src="/assets/block_move_light.gif" alt="Moving a block (Light Mode)" className="block dark:hidden" />
  <img src="/assets/block_move_dark.gif" alt="Moving a block (Dark Mode)" className="hidden dark:block" />
</Frame>

## Deleting Blocks

Right-click on a block and select **Delete** to remove it from the flow.
<Frame>
  <img src="/assets/block_delete_light.gif" alt="Deleting a block (Light Mode)" className="block dark:hidden" />
  <img src="/assets/block_delete_dark.gif" alt="Deleting a block (Dark Mode)" className="hidden dark:block" />
</Frame>

<Callout type="warning">
  Deleting a block cannot be undone. Confirm that downstream connections are no longer required before removing it.
</Callout>

## Connecting Blocks

<Frame>
  <img src="/assets/block_connect_light.gif" alt="Connecting a block (Light Mode)" className="block dark:hidden" />
  <img src="/assets/block_connect_dark.gif" alt="Connecting a block (Dark Mode)" className="hidden dark:block" />
</Frame>

Connections define the data flow between blocks. Here's how to link them:

<CardGroup cols={2}>
  <Card title="Create Connections">
    Click and drag from an output port on one block to an input port on another. Outputs are on the right side, inputs are on the left.
  </Card>
  <Card title="Port Rules">
    Outputs can connect to multiple inputs, but each input can only connect to one output. Ensure data types match for smooth operation.
  </Card>
</CardGroup>

For more details on port data types, refer to the [Block Reference](/block-reference/all-blocks).

## Disconnecting Blocks

To adjust a connection:

- **Move Connection**: Click and drag on a connected port to change the connection to a different port.
- **Delete Connection**: Drag the connection line to an empty space to remove it.

## Creating a Subflow

<Frame>
  <img src="/assets/block_create_subflow_light.gif" alt="Creating a subflow (Light Mode)" className="block dark:hidden" />
  <img src="/assets/block_create_subflow_dark.gif" alt="Creating a subflow (Dark Mode)" className="hidden dark:block" />
</Frame>

Subflows allow you to group blocks for cleaner, more manageable workflows.

<Steps>
  <Step title="Select Blocks">
    Hold **Shift** and click on each block you want to include in the subflow.
  </Step>
  <Step title="Create Subflow">
    Right-click one of the selected blocks and choose **Create Subflow**. This action moves the selected blocks into a new, unsaved flow, complete with input and output blocks that link back to the main flow.
  </Step>
</Steps>

<Tip>
  Name and describe your subflow in the flow info section to keep it organized. Don't forget to save with **CMD+S** or **CTRL+S**.
</Tip>

<Note>
  The original blocks in the parent flow remain in place, so you'll need to manually replace them with your subflow if desired.
</Note>

## Editing a Block

<Frame>
  <img src="/assets/block_edit_light.gif" alt="Editing a block (Light Mode)" className="block dark:hidden" />
  <img src="/assets/block_edit_dark.gif" alt="Editing a block (Dark Mode)" className="hidden dark:block" />
</Frame>

To customize a block, click the gear icon in the top-right corner. This opens the **Block Editor**.

<CardGroup cols={2}>
  <Card title="Edit Block Details">
    Adjust the block's title and description for clear documentation.
  </Card>
  <Card title="Fine-Tune Settings">
    Modify settings specific to the block type, such as batch processing or variant options.
  </Card>
</CardGroup>