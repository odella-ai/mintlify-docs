...
description: "Learn how to debug your workflows in real-time using Odella's live debugging features"
---

Live debugging lets you inspect each block's status and output during a workflow run. Use it to find the first step where actual behavior differs from the expected result.

<Frame>
  <img src="../assets/workflows_live_light.gif" alt="Workflow Live Flow (Light Mode)" className="block dark:hidden" />
  <img src="../assets/workflows_live_dark.gif" alt="Workflow Live Flow (Dark Mode)" className="hidden dark:block" />
</Frame>
