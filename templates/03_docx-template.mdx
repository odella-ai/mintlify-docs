...
description: "Configure Word templates with variables, conditions, loops, equations, and reusable document logic."
---

## Create Your First DOCX Template

Use DOCX templates to turn approved Microsoft Word documents into reusable templates in Odella. Add variables, conditions, loops, equations, and image fields for contracts, letters, reports, correspondence, and other documents with dynamic content.

<Callout type="info" emoji="📄">
  DOCX templates work best for documents with dynamic content. Provide approved source material and review the populated document before final use.
</Callout>
...
<Steps>
  ...
  <Step title="Name and Describe Your Template">
    Give the template a specific name and description. State its purpose, when it applies, when it should not be used, and any review requirements.
  </Step>
  ...
  <Step title="Sync Template Fields">
    After adding Mustache syntax, select **Sync Document**. Odella detects template variables and proposes field types and filling instructions from the variable names and surrounding document context. Review every proposed instruction before publishing.
  </Step>
...
Conditional sections display content only when their condition evaluates to true. Use them for clauses or sections that depend on defined input values.
...
<CardGroup cols={2}>
  ...
  <Card title="Default Values" icon="shield-check">
    Set approved default values for common fields and define fallback content when optional data is not provided.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Batch Document Generation" icon="copy">
    Generate one document or a reviewed batch from validated source data.
  </Card>
...
**You:** "Draft the client engagement letter for Acme Corp using the details from our email thread and the attached proposal"
...
- Generates a document using the template's formatting and configured language
- Delivers the completed document ready for your review

<Callout type="info" emoji="📖">
  For detailed instructions on how to use your published templates with AI Employees and workflows, see our [Using Templates](/templates/04_using-templates) guide. To populate templates inside a workflow, use the [Populate Template block](/block-reference/draft/populate-template).
</Callout>

## Template Examples

<Callout type="tip" emoji="💡">
  The following patterns demonstrate variables, conditions, loops, equations, and comments.
</Callout>
...
{{! Late fee calculation is handled in the approved billing workflow }}
```
...
- [Templates Overview](/templates/01_overview)
- [Using Templates](/templates/04_using-templates)
- [Populate Template Block](/block-reference/draft/populate-template)
- [Text to File Block](/block-reference/draft/text-to-file)
- [Flow Output Block](/block-reference/io/graph-output)

Use representative test data and review every conditional path before publishing a DOCX template for production work.
